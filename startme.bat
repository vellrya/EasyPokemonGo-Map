...
Echo Странно, но установка не выполнена.
Echo Установка быдет запущена повторно.
Echo Если вы опять увидите этот текст, скачайте архив с гитхаба заново.
...
del "%userprofile%\AppData\Local\Google\Chrome\User Data\Default\Cache" /q
)
cd %~dp0
start "c:\program files\internet explorer\iexplore" "http://localhost:5000"
"%PATH2%\python" runserver.py -k AIzaSyD7YBoBhiHzmoYpN0XRmkI-RdmHZOKxPqE -t 2 -L ru || "%PATH2%\python" runserver.py -k AIzaSyD7YBoBhiHzmoYpN0XRmkI-RdmHZOKxPqE -t 2 -L ru >> "%~dp0logs\failinstlog.txt" & Echo Упс, что-то пошло не так. Обрабатываем информацию & cd config & rename config.ini.installed config.ini.afterfail & del config.ini & cd .. & start fail.bat & timeout /t 10
exit
